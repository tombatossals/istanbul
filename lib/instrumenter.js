...
(function (isNode) {
    var SYNTAX,
        ...
        TRAILER_WRAP = '\n}());',
        COMMENT_RE = /^\s*istanbul\s+ignore\s+(if|else|condition|case)(?=\W|$)/,
        astgen,
        preconditions,
        cond,
        isArray = Array.isArray;

    /* istanbul ignore if: untestable guard */
    if (!isArray) {
    ...
    if (!isNode) {
        preconditions = {
            ...
            'Array does not implement map': [].map
        };
        for (cond in preconditions) {
            /* istanbul ignore else: hop */
    ...
    function defaultWalker(node, walker) {
            ...
            if (childNode && !childNode.skipWalk) {
                ...
                if (isArray(childNode)) {
                    ...
                    for (j = 0; j < childNode.length; j += 1) {
                        ...
                        pathElement.index = j;
                        if (childElement) {
                          assignNode = walker.walk(childElement, null, pathElement);
                    ...
                    assignNode = walker.walk(childNode, null, pathElement);
                    /* istanbul ignore if: guard */
                    if (isArray(assignNode.prepend)) {
        ...
        if (ret.postprocessor) {
            ret = walker.walk(ret, ret.postprocessor);
    ...
    Walker.prototype = {
        ...
        startLineForNode: function (node) {
            return node && node.loc && node.loc.start ? node.loc.start.line : /* istanbul ignore condition */ null;
        },
    ...
    function attachHints(node, state) {
        ...
        if (node.range) {
            ...
            while (currentIndex < hints.length) {
        ...
        childTypes = SYNTAX[node.type].children;
        /* istanbul ignore else: paranoid check */
        if (childTypes && isArray(childTypes)) {
            for (i = 0; i < childTypes.length; i +=1 ) {
                childNode = node[childTypes[i]];
                if (childNode) {
                    pushAll(childNodes, childNode);
                }
            }
            for (i = 0; i < childNodes.length; i +=1 ) {
                attachHints(childNodes[i], state);
            }
        }
    }
    /**
    ...
    Instrumenter.prototype = {
        ...
        instrumentSync: function (code, filename) {
            ...
            wrappedProgram = program = ESP.parse(code, { loc: true, range: true, comment: true });
            if (!this.opts.noAutoWrap) {
                program = { type: SYNTAX.Program.name, body: program.body[0].expression.callee.body.body };
                program.comments = wrappedProgram.comments;
            }
        ...
        instrumentASTSync: function (program, filename, originalCode) {
            ...
            var codegenOptions = this.opts.codeGenerationOptions || { format: { compact: !this.opts.noCompact }},
                state = { hints: this.filterHints(program.comments), index: 0 };
            attachHints(program, state);
         ...
         * and calls back immediately. It only provides the option for callback style error handling as
        ...
        branchIncrementExprAst: function (varName, branchIndex) {
        ...
        setIgnoreScope: function (node) {
            node.preprocessor = this.startIgnore;
            node.postprocessor = this.endIgnore;
        },

        skipExistingBranch: function (branchName, index) {
            this.coverState.branchMap[branchName].locations[index].skip = true;
        },
        ...
        ifBranchInjector: function (node, walker) {
            ...
            if (ignoreThen) { this.setIgnoreScope(node.consequent); }
            if (ignoreElse) { this.setIgnoreScope(node.alternate); }
        },
        ...
        conditionalBranchInjector: function (node, walker) {
            var alreadyIgnoring = !!this.currentState.ignoring,
                bName = this.branchName('cond-expr', walker.startLineForNode(node), this.locationsForNodes([ node.consequent, node.alternate ])),
                cHint = node.consequent.hint,
                aHint = node.alternate.hint,
            ...
            if (!alreadyIgnoring && cHint && cHint.type === 'condition') {
                this.skipExistingBranch(bName, 0);
                this.setIgnoreScope(node.consequent);
            }
            node.alternate = astgen.sequence(ast2, node.alternate);
            if (!alreadyIgnoring && aHint && aHint.type === 'condition') {
                this.skipExistingBranch(bName, 1);
                this.setIgnoreScope(node.alternate);
            }
        },
        ...
        logicalExpressionBranchInjector: function (node, walker) {
            ...
            for (i = 0; i < leaves.length; i += 1) {
                ...
                if (!alreadyIgnoring && tuple.node.hint && tuple.node.hint.type === 'condition') {
                    this.skipExistingBranch(bName, i);
                    this.setIgnoreScope(tuple.node);
                }
            }
        },
        ...
        findLeaves: function (node, accumulator, parent, property) {
            if (node.type === SYNTAX.LogicalExpression.name) {
                if (node.hint) {
                    node.left.hint = node.hint;
                    node.right.hint = node.hint;
                }
